...
@ticket_router.patch("/{ticket_id}/book", status_code=status.HTTP_200_OK, response_model=GenericSuccessResponseSchema)
async def book(
    body: TicketBookSchema, ticket_service: TicketServiceDep, user_id: OptionalUserIdDep, ticket_id: Int32Path
) -> GenericSuccessResponseSchema:
...
@ticket_router.patch("/{ticket_id}/pay", status_code=status.HTTP_200_OK, response_model=GenericSuccessResponseSchema)
async def pay(
